...
// Fallback Badge component for direct styling control and fallback functionality
...
export function PlanManager({ 
  ...
  onPlanSelect,
  selectedPlanId 
}: PlanManagerProps) {const [plans, setPlans] = useState<Plan[]>([]);
  ...
  const getStepIcon = (status: Plan['steps'][0]['status']) => {
    switch (status) {
      case 'completed':
  ...
  const debouncedRefresh = useCallback(
    debounce(async () => {
      if (isRefreshing || !currentNamespace) return;
      ...
      try {
        ...
        activePlans.forEach(plan => {
          if (!uniquePlans.has(plan.id) || 
              new Date(plan.updated) > new Date(uniquePlans.get(plan.id)!.updated)) {
        ...
        onError(error instanceof Error ? error.message : 'Failed to fetch plans');
      } finally {
        setIsRefreshing(false);
      }
    }, 500),
    [currentNamespace, config, onError, isRefreshing]
  );
  ...
  (() => {
    ...
    const handlePlanEvent = () => {
      ...
      refreshTimeoutRef.current = setTimeout(debouncedRefresh, 500);
    };
  ...
  }, [currentNamespace, debouncedRefresh]);
  ...
  const handleStepStatusUpdate = async (planId: string, stepId: string, newStatus: Plan['steps'][0]['status']) => {
    ...
    try {
      ...
      window.dispatchEvent(new CustomEvent('planUpdated', { detail: updatedPlan }));
    } catch (error: any) {
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {plans.length === 0 ? (
          <div className="text-center text-muted-foreground py-8">
            {isRefreshing ? 'Loading plans...' : 'No plans created yet'}
          </div>
        ) : (
          plans.map(plan => (
            <div

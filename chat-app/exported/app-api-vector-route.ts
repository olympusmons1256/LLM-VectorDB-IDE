...
function determineSectionByType(type: FileMetadata['type']): FileMetadata['section'] {
  switch (type) {
    ...
    case 'documentation':
      return 'documentation';
    default:
      return 'full';  // Default section for complete files
  }
}
    ...
    switch (operation) {
      case 'describe_namespace': {
        try {
          ...
          // Try getting just one record to examine metadata
          ...
          if (stats.namespaces) {
            for (const [name, data] of Object.entries(stats.namespaces)) {
              if (name && name.trim() !== '') {
                ...
                // Only count complete files for namespace stats
                const completeFiles = await indexNs.query({
                  vector: createInitializationVector(),
                  filter: { 
                    isComplete: { $eq: true },
                    section: { $ne: 'chunks' }  // Exclude chunks from counts
                  },
                  includeMetadata: true,
                  topK: MAX_QUERY_RESULTS
                });
      ...
      case 'delete_document': {
        ...
        try {
          ...
          // Query both full files and chunks
      ...
      case 'process_document': {
        ...
        try {
          ...
          // Create chunks and add delays for indexing
          ...
          // Add a delay after processing for indexing
          await new Promise(resolve => setTimeout(resolve, 1000));
      ...
      case 'query_context': {
        ...
        try {
          ...
          // Allow plans section in addition to full and chunks
          const sectionFilter = {
            $or: [
              { section: { $eq: 'full' } },
              { section: { $eq: 'chunks' } },
          ...
          if (filter) {
            // If a custom filter is provided, use it instead of the default section filter
